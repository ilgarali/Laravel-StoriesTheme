    ...
    <div class="container">
        <div class="row no-gutters slider-text align-items-end justify-content-center">
            <div class="col-md-9 ftco-animate pb-5 text-center">
                <h1 class="mb-3 bread">{{$category->category}}</h1>
                <p class="breadcrumbs"><span class="mr-2"><a href="index.html">Home <i class="ion-ios-arrow-forward"></i></a></span> <span>{{$category->category}} <i class="ion-ios-arrow-forward"></i></span></p>
            </div>
        </div>
    </div>
</section>
                    ...
                    <div class="col-md-6 col-lg-12 ftco-animate">
                        <div class="blog-entry d-lg-flex">
                            ...
                            <div class="text px-md-4 px-lg-5 half pt-3">
                                <p class="meta d-flex"><span class="pr-3">{{$category->category}}</span><span class="ml-auto pl-3">March 01, 2018</span></p>
                                <h3><a href="{{route('single',[$post->category->slug,$post->slug])}}"> {{$post->title}}  </a></h3>
                                <p> {{\Illuminate\Support\Str::words($post->content,20)}}
                                </p>
                                <p class="mb-0"><a href="{{route('single',[$post->category->slug,$post->slug])}}" class="btn btn-primary">Read More <span class="icon-arrow_forward ml-4"></span></a></p>
                            </div>
                        </div>
                    </div>
        @endforeach
